...
def generate_rag_response(db: Session, user_input: str, model: str) -> str:
    try:
        ...
        results = db.query(
            Vector.content,
            Vector.embedding.cosine_distance(query_vector).label('distance') # 임베딩 유사도 검색
        ...
        if model == "gpt":
            ...
            return generate_local_llm_chat_response("qwen2.5:7b", messages)
        elif model == "gemma3:4b":
            return generate_local_llm_chat_response("gemma3:4b", messages)
        else:
...
def generate_gemini_chat_response(messages: list[dict]) -> str:
    try:
        ...
        for msg in messages:
            ...
            if role == 'system':
                prompts.append(f"Sysetm: {content}")
            elif role == 'user':
...
def generate_sql_response(db: Session, user_input: str, model: str) -> dict:
    sql = generate_sql_from_text(db, user_input, model)
    print(sql)
    execution_result = execute_generated_sql(db, sql)
